  ...
  try {
    ...
    const transformed = data.map(perm => {
      ...
      const actionMapping: Record<string, string> = {
        ...
        'status-sales-approved': 'sales-approval',
        'status_updates-sales_approval': 'sales-approval',
        'case-status-sales-approval': 'sales-approval',
        'case-sales-approved': 'sales-approval',
...
export const saveSupabasePermissions = async (permissions: Permission[]): Promise<boolean> => {
  try {
    ...
    const { error: deleteError } = await supabase
      .from('permissions')
      .delete()
      .gte('id', 1); // Delete all rows (safe condition that matches all records)
